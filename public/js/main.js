...
(function ($) {
  ...
  var rtsJs = {
    ...
    ensureMobileMenuButton: function () {
      $(document).ready(function () {
        $('.header-one .header-main-one-wrapper').each(function () {
          const $wrapper = $(this);
          if ($wrapper.find('.menu-btn').length) return;

          const hasGlobalId = $('#menu-btn').length === 0;
          const btnIdAttr = hasGlobalId ? 'id="menu-btn"' : '';
          const buttonMarkup = `
            <div class="button-area d-lg-none mobile-menu-trigger">
              <button ${btnIdAttr} type="button" class="menu menu-btn ml--20 ml_sm--5" aria-label="Open menu">
                <img class="menu-light" src="assets/images/icons/01.svg" alt="Menu-icon">
              </button>
            </div>`;

          const $mainHeader = $wrapper.find('.main-header').first();
          if ($mainHeader.length) {
            $mainHeader.after(buttonMarkup);
          } else {
            $wrapper.append(buttonMarkup);
          }
        });
      });
    },
